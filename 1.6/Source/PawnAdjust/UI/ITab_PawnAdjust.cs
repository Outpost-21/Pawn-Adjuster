    ...
    public class ITab_PawnAdjust : ITab
    {
            ...
            if (!selectorOpen)
            {
                foreach (Apparel item in SelPawn.apparel.WornApparel)
                {
                    ...
                    if (story != null && story.favoriteColor != null)
                    {
                        ...
                        {
                            item.SetColor(story.favoriteColor.color);
                            SelPawn.drawer.renderer.SetAllGraphicsDirty();
                            SoundDefOf.Tick_Low.PlayOneShotOnCamera();
                        }
                        flag = true;
        ...
        public List<Color> GetAllColors()
        {
            if (allColors == null)
            {
                ...
                if (SelPawn.story != null && !SelPawn.DevelopmentalStage.Baby() && SelPawn.story.favoriteColor != null && !allColors.Any((Color c) => SelPawn.story.favoriteColor.color.IndistinguishableFrom(c)))
                {
                    allColors.Add(SelPawn.story.favoriteColor.color);
                }
