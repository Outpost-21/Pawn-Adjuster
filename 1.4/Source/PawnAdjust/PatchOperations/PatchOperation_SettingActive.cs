...
namespace PawnAdjust
{
    public class PatchOperation_SettingActive : PatchOperation
    {
        ...
        public override bool ApplyWorker(XmlDocument xml)
        {
            ...
            ModContentPack modContentPack = PawnAdjustMod.mod.Content;
            for (int i = 0; i < settings.Count(); i++)
            {
                if (!PawnAdjustMod.settings.IsValidSetting(settings[i]))
                {
                ...
                if (PawnAdjustMod.settings.GetEnabledSettings.Contains(settings[i]))
                {
                    flag = true;
                    break;
                }
